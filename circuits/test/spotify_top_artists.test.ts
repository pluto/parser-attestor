...
describe("spotify top artists", async () => {
    ...
    it("POST response body extraction", async () => {
        ...
        let [input, key, output] = readJSONInputFile(
            `${jsonFilename}.json`,
            [
                "data",
                "me",
        ...
        // const params = [input.length, http.bodyBytes.length, lockData.version.length, lockData.status.length, lockData.message.length];
        // headers.forEach(header => {
        //     params.push(header[0].length);
        //     params.push(header[1].length);
        // });

        // console.log("starting witness gen");
